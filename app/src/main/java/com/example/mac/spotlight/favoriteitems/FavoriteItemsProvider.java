...
public class FavoriteItemsProvider extends ContentProvider {
	private static final String AUTHORITY = "spotlight.minor.spotlight.favoriteitems.FavoriteItemsProvider";
	...
	private void insert(SQLiteDatabase db, ContentValues values) {
		...
		if (id >= 0) {
			db.execSQL("UPDATE " + FAVORITE_ITEMS_TABLE_NAME + " SET Counter = " + ++counter + " WHERE SearchText = '" + searchText + "' AND _ID = " + id);
		}
